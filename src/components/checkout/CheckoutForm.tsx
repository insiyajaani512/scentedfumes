...
import { processCheckout, applyManualCoupon } from "@/lib/graphql/checkout";
...
export default function CheckoutForm() {
  ...
  const handleApplyPromo = async () => {
    ...
    try {
      const result = await applyManualCoupon(code);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const invalidItems = items.filter(
      (item) => !item.databaseId || item.databaseId <= 0
    );
    ...
    try {
      const testerSelectionsNote = items
        .filter(
          (item) =>
            item.testerSelections && item.testerSelections.length > 0
        )
        .map((item) => {
          const selections = item.testerSelections
            ?.map(
              (selection, index) =>
                `Tester ${index + 1}: ${selection}`
            )
            .join(" | ");
      ...
      const promotionNote = promotion
        ? `Promotion: ${promotion.label} | Items: ${promotion.selections
            .map(
              (id) =>
                items.find((it) => it.databaseId === id)?.name ??
                String(id)
            )
            .join(", ")}`
        : "";
    ...
    return (
      <div className="rounded-2xl border border-[var(--accent-gold)]/15 bg-[var(--bg-main)]/60 p-8 text-center">
        <p className="text-[var(--text-secondary)]">
          Your cart is empty. Browse our collection to find your signature
          scent.
        </p>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      <section className="flex flex-col" style={{ gap: "1rem" }}>
        <div>
          <p className="text-[var(--accent-gold)]">Billing details</p>
          <h2
      ...
      <section className="flex flex-col" style={{ gap: "1rem" }}>
        <div>
          <p className="text-[var(--accent-gold)]">Shipping address</p>
          <h2
            className="text-[var(--text-primary)]"
            style={{
              fontFamily: "var(--font-playfair)",
      ...
      <section className="rounded-2xl border border-[var(--accent-gold)]/15 bg-[var(--bg-main)]/60 p-5">
        <div className="mb-4">
          <p className="text-[var(--accent-gold)]">Promo code</p>
          <h2
            className="text-[var(--text-primary)]"
            style={{
              fontFamily: "var(--font-playfair)",
        ...
        {promoError && (
          <p className="mt-3 text-red-400">{promoError}</p>
        )}
      ...
      <section className="rounded-2xl border border-[var(--accent-gold)]/15 bg-[var(--bg-main)]/60 p-5">
        <p className="text-[var(--accent-gold)]">Payment</p>
        <h2
      ...
      <section className="rounded-2xl border border-[var(--accent-gold)]/15 bg-[var(--bg-main)]/60 p-5">
        <p className="text-[var(--accent-gold)]">Order summary</p>
        <h2
          className="mb-4 text-[var(--text-primary)]"
          style={{
            fontFamily: "var(--font-playfair)",
          ...
          {items.map((item) => (
            ...
            >
              ...
              <strong className="text-[var(--text-primary)]">
                Rs{" "}
                {(
                  extractNumericPrice(item.price) *
                  (item.quantity || 0)
                ).toLocaleString()}
              </strong>
            </div>
        ...
        >
          ...
          {appliedPromoCode && (
            <p
              ...
              style={{ opacity: 0.7 }}
            >
              Final coupon discount is calculated by WooCommerce when the
              order is placed.
            </p>
          )}
        </div>
